...
namespace EmailValidator.Controllers
{
    public class ValidationData
    {
        ...
        public bool HasProbableName { get; set; }
    ...
    public class ValidationResult
    {
        ...
        public bool DomainExists { get; private set; }
        public bool Probable { get; private set; }
        ...
        public ValidationData Result { get; private set; }

        private static Regex emailRegEx = new Regex("^[_a-z0-9-]+(.[a-z0-9-]+)@[a-z0-9-]+(.[a-z0-9-]+)*(.[a-z]{2,4})$", RegexOptions.Compiled);
        
        private bool RegExValidate(string email)
        {
            return emailRegEx.IsMatch(email);
        }

        private string ReplaceAndRemoveWhiteSpaceBefore(string email, string part, string newpart)
        {
            var idx = email.IndexOf(part);
            var end = "";
            if (idx + part.Length < email.Length)
            {
                end = email.Substring(idx + part.Length).Trim();
            }
            return email.Substring(0, idx).Trim() + newpart + end;
        }

        internal static Dictionary<string, string> SpellReplace = new Dictionary<string, string>()
        {
            { ". om",".com" },
            { "htomail.com,","hotmail.com" }
        };

        private string FixCommon(string email)
        {
            if (email.Contains("@"))
            {
                var dp = email.Split('@');
            ...
            {
                var isFixed = false;
                foreach (var chr in domainSplitChars)
                {
                    string check = chr + td;
                    if (email.EndsWith(check))
                    {
                        email = ReplaceAndRemoveWhiteSpaceBefore(email, check, "." + td);
                        isFixed = true;
                        break;
                    }
                }
        ...
        private void PopulateData(string email)
        {
            if (Result == null)
                Result = new ValidationData();
            var parts = email.Split('@');
            var name = parts[0];
            var splitChar = GetSplitChar(name);
        ...
        private static char GetSplitChar(string name)
        {
            var splitChar = '.';
        ...
        private string CleanSpelling(string email)
        {
            ...
            if (!email.Contains("@"))
            {
                foreach (var chr in new char[] { '&', '#', '"', '2', '¤', '-', ',', '.' })
                {
                    var chrIdx = email.IndexOf(chr);
                    if (chrIdx != -1)
                    {
                        ret = email.Substring(0, chrIdx).Trim() + "@" + email.Substring(chrIdx + 1).Trim();
                        break;
                    }
                }
            }
        ...
        private string CleanDomain(string email)
        {
            ...
            var topDomain = domainParts.LastOrDefault();
            if (!string.IsNullOrEmpty(topDomain))
            {
                if (!IsValidTopDomain(topDomain))
                {
                    foreach (var chr in domainSplitChars)
                    {
                        if (topDomain.Contains(chr))
                        {
                            var newTop = topDomain.Substring(topDomain.LastIndexOf(chr));
                            if (IsValidTopDomain(newTop))
                            {
                                ret = ret.Replace(topDomain, "." + newTop);
                            }
                        }
                    }
                }
